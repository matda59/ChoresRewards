document.addEventListener('DOMContentLoaded', function() {
    ...
    if (topTasksTrendCtx && chartData.topTasksTrendData && Object.keys(chartData.topTasksTrendData).length > 0) {
        // top_tasks_trend is keyed by person name: {person: {labels, datasets}}
        // Merge all persons' datasets into a single chart
        const trendByPerson = chartData.topTasksTrendData;
        const firstPerson = Object.keys(trendByPerson)[0];
        const mergedTrendData = {
            labels: trendByPerson[firstPerson].labels,
            datasets: []
        };
        for (const [person, personData] of Object.entries(trendByPerson)) {
            for (const dataset of personData.datasets) {
                mergedTrendData.datasets.push(Object.assign({}, dataset, {
                    label: Object.keys(trendByPerson).length > 1
                        ? person + ': ' + dataset.label
                        : dataset.label
                }));
            }
        }
        topTasksTrendChart = createTopTasksTrendChart(topTasksTrendCtx, mergedTrendData);
    }
});
